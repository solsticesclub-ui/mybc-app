...
export async function POST() {
  ...
  await supabase.from("reports").update({ generation_status: "generating_mission" }).eq("user_id", user.id);

  const existingData = report.data as ReportData;
  const raw = await callClaude(promptStep4(profile, chartSummaryFromData(existingData)));
